...
public final class HistoryScreen {
    ...
    private final Map<HistoryFilter, ToggleButton> filterButtons = new java.util.EnumMap<>(HistoryFilter.class);
    ...
    public void applyLanguage(AppLanguage language) {
        ...
        resultColumn.setText(language == AppLanguage.ENGLISH ? "Result / conf." : "Résultat / conf.");
    ...
    public void refresh() {
        List<RunEvent> loaded = new java.util.ArrayList<>(store.readAll());
    ...
    private void confirmAndClearHistory() {
        ...
        dialog.getIcons().add(com.episort.ui.AppShell.logoImage());
    ...
    private static String normalizeSearchText(String text) {
        ...
        String decomposed = Normalizer.normalize(text.trim().toLowerCase(java.util.Locale.ROOT), Normalizer.Form.NFD);
    ...
    private void configureTable() {
        ...
        sourceColumn.setCellValueFactory(data -> new SimpleStringProperty(
                data.getValue().subjectPath().map(Path::toString).orElse(EMPTY)));
    ...
    private static String resultText(RunEvent event) {
        ...
        if (!EMPTY.equals(confidence)) {
            return confidence;
        }
        return event.summary().isBlank() ? EMPTY : event.summary();
    }
