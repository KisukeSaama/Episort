...
final class HistoryDetailPanel {
    ...
    private void applyFieldLabels(AppLanguage language) {
        ...
        setFieldLabel(summarySection, 2, UiText.historyDetailFieldWorkspace(language));
    ...
    void show(RunEvent event, AppLanguage language) {
        ...
        workspaceValue.setText(event.workspace().map(path -> path.toAbsolutePath().normalize().toString()).orElse(UiText.EMPTY));
        sourceValue.setText(event.subjectPath().map(path -> path.toAbsolutePath().normalize().toString()).orElse(UiText.EMPTY));
        statusValue.setText(HistoryScreen.statusText(event.status(), language));
        summaryValue.setText(event.summary().isBlank() ? UiText.EMPTY : event.summary());

        metricsList.getChildren().clear();
        if (event.metrics().isEmpty()) {
            Label emptyMetrics = new Label(UiText.EMPTY);
