...
record BatchTmdbMatch(String seedName, InventoryGroupType type, int itemCount, boolean finalMatch) {

    static List<BatchTmdbMatch> from(InventoryScanResult result) {
        Objects.requireNonNull(result, "result");
        return result.groups().stream()
                .map(BatchTmdbMatch::from)
                .toList();
    }

    private static BatchTmdbMatch from(InventoryGroup group) {
    ...
    /** Whether the group can carry a TMDB identity at all. */
    boolean namesAMedia() {
    ...
    String statusText(AppLanguage language) {
        ...
        return finalMatch
                ? UiText.scanBatchTmdbStatusFinal(language)
                : UiText.scanBatchTmdbStatusUnresolved(language);
    }
}
