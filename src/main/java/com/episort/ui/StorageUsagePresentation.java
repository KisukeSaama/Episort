...
record StorageUsagePresentation(String percentage, String capacity, String available, double progress) {
    ...
    static StorageUsagePresentation from(Optional<VolumeSpace> volume, AppLanguage language) {
        ...
        if (volume.isEmpty() || volume.orElseThrow().totalBytes() == 0) {
            return new StorageUsagePresentation(UiText.EMPTY, UiText.EMPTY, UiText.EMPTY, 0);
        }
