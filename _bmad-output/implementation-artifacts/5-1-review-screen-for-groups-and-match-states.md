...
1. Given inventory and match proposals exist
...
- Reused the existing virtualized scan `TableView` as the Epic 5 review surface instead of creating a parallel screen.
- Added a workflow-layer review session model that snapshots current review rows without mutating the filesystem.
- Manual verification: load a mixed folder and confirm series, movies, ignored, unsupported, conflicts, and unknowns remain reviewable in the table before validation.
