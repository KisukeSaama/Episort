...
1. Given a pattern has or has not been validated
...
Review `4-4-pattern-validation-gate.md` before implementation. Reuse its established file locations, test patterns, and decisions; do not duplicate or contradict them.
...
- Added exact-plan gate state separately from pattern validation and prevented plan validation before a plan exists.
- Execution eligibility now explains the remaining blockers with stable codes (`PATTERN_NOT_VALIDATED`, `EXACT_PLAN_MISSING`, `EXACT_PLAN_NOT_VALIDATED`).
- Preserved validation across equivalent review-state refreshes so normal navigation does not silently reset the gate.
- Manual verification: validate a pattern, navigate away and back, then confirm the UI still explains that exact-plan review is the remaining blocker.

### File List

- `src/main/java/com/episort/workflow/ReviewSession.java`
- `src/main/java/com/episort/workflow/ReviewValidationSnapshot.java`
- `src/main/java/com/episort/workflow/ExecutionEligibility.java`
