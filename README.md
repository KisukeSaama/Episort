...
Local AI uses Qwen3 4B Instruct 2507 Q4_K_M through an embedded llama.cpp runtime with a 32K context and `/no_think` prompts. The model is downloaded once to `%LOCALAPPDATA%\Episort\models\`. If the runtime or model is unavailable, the app remains usable without AI features.
